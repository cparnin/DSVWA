...
# --- BEGIN TEST SECRETS FOR DEMO PURPOSES ---
...
content = subprocess.check_output(["nslookup", params["domain"]], shell=False, stderr=subprocess.STDOUT, stdin=subprocess.PIPE).decode()
                elif "xml" in params:
...
cursor.execute("INSERT INTO comments VALUES(NULL, :comment, :time)", {"comment": params["comment"], "time": time.ctime()})
...
exec(program, {"__builtins__": {}}, envs)
...
cursor.execute("SELECT * FROM users WHERE username=:username AND password=:password", {"username": params.get("username", ""), "password": params.get("password", "")})
